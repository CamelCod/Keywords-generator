...
6. If an "Approved Keywords" list is provided by the user, you must prioritize including those keywords in your output if they are relevant to the document. You should still generate other relevant keywords as well.
7. The final output must be a JSON object with a single key "keywords" containing an array of strings.`;
      ...
      let approvedKeywordsText = '';
      if (approvedKeywords.trim() !== '') {
        approvedKeywordsText = `\n\nApproved Keywords to prioritize: ${approvedKeywords.trim()}`;
      }

      const userPrompt = `Generate keywords for the following document:\n\nTitle: ${title}${attributesText}\n\nDescription: ${content}${approvedKeywordsText}`;
  ...
  const TabButton: React.FC<{tabId: string, activeTab: string, onClick: (tabId: string) => void, children: React.ReactNode}> = ({ tabId, activeTab, onClick, children }) => (
    <button
      ...
      className={`px-4 py-3 text-lg font-medium transition-colors focus:outline-none ${
        activeTab === tabId
        ...
        <div className="max-w-4xl mx-auto mb-8">
            <div className="border-b border-slate-200">
                <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                    <TabButton tabId="single" activeTab={activeTab} onClick={setActiveTab}>Single Generation</TabButton>
                    <TabButton tabId="bulk" activeTab={activeTab} onClick={setActiveTab}>Bulk Generation (CSV)</TabButton>
                </nav>
            </div>
        </div>
              ...
              <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-slate-200 animate-slide-in-up">
                <form onSubmit={handleGenerateKeywords} className="space-y-6">
                  ...
                  <div>
                   ...
                   <div>
                    <label htmlFor="approved-keywords" className="block text-lg font-semibold text-slate-700">Approved Keywords (Optional)</label>
                    <p className="text-sm text-slate-500 mt-1">Provide a semi-colon (;) separated list of keywords or phrases you want the AI to prioritize.</p>
                    <textarea
                      id="approved-keywords"
                      value={approvedKeywords}
                      onChange={(e) => setApprovedKeywords(e.target.value)}
                      rows={3}
                      className="mt-2 block w-full px-4 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                      placeholder="e.g., quality control; oeps enhancements; liner hanger"
                    ></textarea>
                  </div>
